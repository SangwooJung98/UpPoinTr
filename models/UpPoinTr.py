...
from models.UpPoinTr_utils import *
...
class UpTransformer(nn.Module):
    ...
    def forward(self, pos, key, query, upfeat):
        ...
        """
        #print("Up Transformer inputs", pos.shape, key.shape, query.shape, upfeat.shape)
        ...
        #print("Transformer", pos_flipped.shape)
...
class UpLayer(nn.Module):
    ...
    def __init__(self, dim, seed_dim, up_factor=2, i=0, radius=1, n_knn=20, interpolate='three', attn_channel=True):
        ...
        self.mlp_1 = MLP_CONV(in_channel=3, layer_dims=[64, 64])
        self.mlp_2 = MLP_CONV(in_channel=64 * 2 + seed_dim, layer_dims=[128, dim])
    ...
    def forward(self, pcd_curr, pcd_prev, feat, K_curr=None):
        """
        Args:
            pcd_prev: Tensor, (B, N_prev, 3)
            coarse = B N 3
            feat: B N M
        ...
        """
        B,N,M = feat.shape
        #print("Up Layer inputs",coarse.shape, pcd_prev.shape, feat.shape, K_prev.shape)
        #pcd_prev = pcd_prev.transpose(-1, -2).contiguous()
        # Collect seedfeature
        if self.interpolate == 'nearest':
            idx = knn_point_dis(1, pcd_prev, coarse)
            feat_upsample = index_points(feat, idx).squeeze(2) # (B,  N, M)
        elif self.interpolate == 'three':
            # three interpolate
            #idx, dis = knn_point_dis(3, pcd_prev, coarse, return_dis=True) # (B, N_prev, 3), (B, N_prev, 3)
            #print("PCD ", pcd_prev.shape, coarse.shape)
            idx, dis = get_nearest_index(pcd_curr, pcd_prev, k=3, return_dis=True)
            dist_recip = 1.0 / (dis + 1e-8)
            norm = torch.sum(dist_recip, dim=2, keepdim=True) # (B, N_prev, 1)
            weight = dist_recip / norm # (B, N_prev, 3)
            feat_check = indexing_neighbor(feat, idx) # (B, N, 3, M) -> (B, M, N, 3)
            #print(feat_check.shape, weight.shape)
            feat_upsample = torch.sum( feat_check* weight.unsqueeze(1), dim=-1) #(B, M, N)
        else:
            raise ValueError('Unknown Interpolation: {}'.format(self.interpolate))

        # Query mlps
        
        feat_1 = self.mlp_1(pcd_curr)
        #print("Feat1",feat_1.shape, feat_upsample.shape)
        ...
        H = self.uptrans1(pcd_curr, K_curr if K_curr is not None else Q, Q, upfeat=feat_upsample) # (B, 384, N_prev)
        #print("H", H.shape)
        feat_child = self.uptrans2(pcd_curr, K_curr if K_curr is not None else H, H, upfeat=feat_upsample) # (B, 384, N_prev*up_factor)
        ...
        pcd_new = self.upsample(pcd_curr)
        pcd_new = pcd_new + delta
        #pcd_new = pcd_new.transpose(-1, -2).contiguous()
        #print("PCD_new", pcd_new.shape)
...
class DGCNN_Grouper(nn.Module):
    def __init__(self, k = 16):
        ...
        self.input_trans = nn.Sequential(nn.Conv1d(3,16, 1),
                                        nn.GroupNorm(4, 16),
                                        nn.LeakyReLU(negative_slope=0.2)
                                        )
        self.layer1 = nn.Sequential(nn.Conv2d(32, 32, kernel_size=1, bias=False),
                                   nn.GroupNorm(4, 32),
                                   nn.LeakyReLU(negative_slope=0.2)
                                   )
...
class SimpleRebuildFCLayer(nn.Module):
    ...
    def forward(self, rec_feature):
        ...
        #print(patch_feature.shape, rec_feature.shape, self.step, self.input_dims)
...
class UpPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.increase_dim_base = nn.Sequential(
        ...
        for i in range(len(self.up_factors)):
            if self.up_factors[i]>1 or i==0:
                self.up_blocks.append(UpLayer(dim=decoder_config.embed_dim, seed_dim=decoder_config.embed_dim, up_factor=self.up_factors[i], i=1, n_knn=20, radius=1, 
    ...
    def get_loss(self, ret, gt, epoch=1):
        pred_coarse, pcd, pred_fine = ret
        assert pred_fine.size(1) == gt.size(1)

        
        #intermediate loss
        loss_intermediate = 0
        num_pts = self.num_query
        ...
        loss_recon = loss_coarse + loss_intermediate + loss_fine 

        return loss_coarse, loss_recon
    ...
    def forward(self, xyz):
        ...
        global_feature = self.increase_dim_base(x) # B 1024 N 
        ...
        # forward decoder
        q = self.decoder(q=q, v=mem, q_pos=coarse, v_pos=coor) # b n c
        denoise_length = 0
        

        #UpSampling from coarse point cloud
        seed_pcd = coarse.permute(0,2,1).contiguous() # b 3 n
        seed_feat = q.permute(0,2,1).contiguous() # b c n
        
        B,_,_ = q.shape
        curr_pcd = coarse.permute(0,2, 1).contiguous() # b 3 n
        curr_feat = q.permute(0,2, 1).contiguous() # b c n
        intermediate_pcds = []
        for i, block in enumerate(self.up_blocks):
            curr_pcd, curr_feat = self.up_blocks[i](curr_pcd, seed_pcd, seed_feat, curr_feat)
            if i< len(self.up_blocks)-1:
                intermediate_pcds.append(curr_pcd.permute(0,2,1).contiguous()) #[b n1 3]
        curr_pcd = curr_pcd.permute(0,2,1).contiguous() # b N 3
        
        if self.training:
            assert denoise_length == 0
            assert curr_pcd.size(1) == self.num_query * self.factor
            assert coarse.size(1) == self.num_query

            ret = (coarse, intermediate_pcds, curr_pcd)
            return ret
